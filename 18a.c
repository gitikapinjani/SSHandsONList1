...
int main() {
   ...
   lockCourse(fd, rollNo);
    ...
    lseek(fd, COURSE_RECORD_SIZE * (rollNo - 1), SEEK_SET);
    write(fd, &db2[rollNo - 1], sizeof(db2[0]));
	//sleep(5);
...
gitikapinjani@gitikapinjani-Victus-by-HP-Gaming-Laptop-15-fa0xxx:~/HandsOnList1-SS$ cc 18a.c
gitikapinjani@gitikapinjani-Victus-by-HP-Gaming-Laptop-15-fa0xxx:~/HandsOnList1-SS$ ./a.out
Enter course number (1-3) to lock: 3
process attempting to lock course
process accquired lock on course 3
enter to unlock course 

Process released lock on course 3.
course no: 3
roll no: 1
